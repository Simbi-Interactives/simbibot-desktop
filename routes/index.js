...
router.get("/export_results/examination", async (req, res) => {
  try {    
    dataSyncService
...
router.get("/export_results/evaluation", async (req, res) => {
  try {   
    dataSyncService
...
router.get("/fetch_super_exams", (req, res) => {
  try {
    db.serialize(() => {
      db.all(
        `select * from superexams where rank='first_level' and name NOT IN ('schools', 'Post utme', 'Diploma', 'NECO')`,
        [],
        (err, data) => {
          if (err) {
            console.log(err);
...
router.get("/fetch_post_utme", (req, res) => {
  db.serialize(() => {
    db.all(
      `select * from superexams where rank='second_level'`,
      [],
      (err, data) => {
        if (err) {
          console.log(err);
...
router.get("/fetch_questions_from_exam_list", (req, res) => {
  ...
  db.serialize(() => {
    db.all(
      `SELECT * FROM Questions WHERE subject_id=${subject_id} AND super_exam_id=${super_exam_id} and deleted_at is null ORDER BY RANDOM() LIMIT 20;`,
      [],
      async (err, data) => {
        let questions = [];
        if (data.length > 0) {
...
router.get("/fetch_questions_from_exam_list_count", (req, res) => {
  ...
  db.serialize(() => {
    db.all(
      `SELECT count(*) FROM Questions WHERE subject_id=${subject_id} AND super_exam_id=${super_exam_id}`,
      [],
      (err, data) => {
        if (err) {
          console.log(err);
...
router.post("/update_evaluation_result", (req, res) => {
  ...
  db.serialize(() => {
    db.run(
      `update evaluations set score=${body.score}, end_time='${body.end_time}', completed_at='${body.completed_at}', status='${body.status}' where id=${body.id}`,
      function (err) {
        if (err) {
          console.log(err);
          return res.status(422).send(err);
...
router.post("/update_examination_result", (req, res) => {
  ...
  db.serialize(() => {
    db.run(
      `update examattempts set score=${body.score}, end_time='${body.end_time}', completed_at='${body.completed_at}', recommended_topic='${body.recommended_topic}' where id=${body.id}`,
      function (err) {
        if (err) {
          console.log(err);
          return res.status(422).send(err);
...
router.get("/user_evaluation_report_avg/:user_id", (req, res) => {
  ...
  db.serialize(() => {
    db.all(
      `SELECT *, COUNT( * ) as attempts, AVG(score) as aggregate_score FROM evaluations JOIN topics on evaluations.topic_id = topics.id JOIN Subjects on evaluations.subject_id = Subjects.id where user_id =${user} Group By evaluations.topic_id`,
      (err, data) => {
        if (err) {
          console.log(err);
          return res.status(422).send(err);
...
router.get(
  "/user_examination_all/:user_id/:exam_id/:subject_id",
  (req, res) => {
    const user_id = req.params.user_id;
...
router.get(
  "/user_evaluation_all/:user_id/:subject_id/:topic_id",
  (req, res) => {
    const user_id = req.params.user_id;
    const subject_id = req.params.subject_id;
    const topic_id = req.params.topic_id;
    ...
    db.serialize(() => {
      db.all(
        `select * from evaluations join subjects on evaluations.subject_id = subjects.id join topics on evaluations.topic_id = topics.id where user_id=${user_id}  AND topic_id=${topic_id}`,
        (err, data) => {
          if (err) {
            console.log(err);
            return res.status(422).send(err);
...
router.get("/evaluations_by_subject_for_line_chart/:subject_id", (req, res) => {
  ...
  db.serialize(() => {
    db.all(
      `select * from evaluations inner join users on evaluations.user_id = users.id join topics on evaluations.topic_id = topics.id  where evaluations.subject_id=${subject_id} group by evaluations.topic_id`,
      [],
      (err, data) => {
        if (err) {
          console.log(err);
...
router.get("/evaluations_avg_score_on_subject/:subject_id", (req, res) => {
  ...
  db.serialize(() => {
    db.all(
      `select *, evaluations.user_id as uid, count (*) as attempts, AVG(score) as aggregate_score FROM evaluations Inner JOIN users on evaluations.user_id = users.id JOIN subjects on evaluations.subject_id = subjects.id JOIN  topics on evaluations.topic_id = topics.id where evaluations.subject_id = ${subject_id}  Group By evaluations.topic_id `,
      (err, data) => {
        if (err) {
          console.log(err);
          return res.status(422).send(err);
...
router.get("/examinations_by_subject_for_line_chart/:exam_id", (req, res) => {
  ...
  db.serialize(() => {
    db.all(
      `select * from examattempts join superexams on examattempts.exam_id = superexams.id join subjects on examattempts.subject_id = subjects.id where examattempts.exam_id = ${exam_id} group by examattempts.subject_id`,
      (err, data) => {
        if (err) {
          console.log(err);
          return res.status(422).send(err);
...
router.get("/examination_avg_score_on_exams/:exam_id", (req, res) => {
  ...
  db.serialize(() => {
    db.all(
      `select *, count(*) as attempts, AVG(score) as aggregate_score from examattempts join subjects on examattempts.subject_id = subjects.id join topics on examattempts.recommended_topic = topics.id left join users on examattempts.user_id = users.id  where examattempts.exam_id = ${exam_id} group by examattempts.subject_id`,
      (err, data) => {
        if (err) {
          console.log(err);
          return res.status(422).send(err);
...
router.post("/show_settings", (req, res) => {
  ...
  db.serialize(() => {
    db.run(
      `update superexams set show = ${body.value} where id = ${body.id}`,
      (err) => {
        if (err) {
          console.log(err);
          return res.status(422).send(err);
...
router.get("/set_novel/:id", (req, res) => {
  ...
  db.run(
    `update settings set should_show_novel = ${update} where id = 1`,
    (err) => {
      if (err) {
        console.log(err);
        return res.status(422).send(err);
