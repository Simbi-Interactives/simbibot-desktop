...
router.post('/login', async (req, res) => {
  try {
    ...
    // console.log(data);
    db.serialize(() => {
        db.all(`select * from users where email='${body.email}'`, [], async (err, data) => {
          if(err) {
            console.log(err);
            return res.status(422).send(err);
          }
  ...
  db.serialize(() => {
    ...
    db.all(`select count(id)  from users`, [], (err, data) => {
      ...
      db.all(`select * from users limit ${limit} offset ${offset}`, [], (err, users) => {
        ...
        return res.status(200).json({data: users,  page, total: pages, limit, offset, metric: count });
      });
    })
...
router.post('/bulk_upload_student', upload.single('file'), async (req, res) => {
  ...
  if(studentsArray[0].firstname.includes('firstname')) studentsArray.splice(0, 1);

  try {
    async.each(studentsArray, async (student, callback) => {
      db.serialize(async () => {
        try {
          const salt = await bcrypt.genSalt(5)
